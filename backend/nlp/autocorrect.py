...
CLINICAL_AUTOCORRECT_RULES = [
    ...
    # Panadol: ALL known Whisper phonetic outputs (add new ones here as discovered)
    (r"(?:پلڈٹال|پنڈال|پینڈال|پینڈول|پیناڈول|پینا\s*ڈول|پینادول|پینڈڈال|پنادول|پنڈول|پندال|پناڈول|پنڈٹال|پلنڈال|پینڈل|پلڈال)", "Panadol"),
    ...
    (r"(?:اگمانٹن|اوگمینٹن|اوگمنٹن|اوگمنٹین|اگمنٹن|اگمنٹین|اگمینٹن|اگمینٹین|آگمنٹن|آگمینٹن|اسکا\s*بم|اسکھابم|اسکابم|اوگمانٹن|آگمانٹن)", "Augmentin"),
    ...
    # CRITICAL: مج / ملے گرام / ملگرام are all common Whisper outputs for "mg"
    (r"(\d+)\s*(?:مج[ی]?|ملج|ملے\s*گرام|ملک\s*گرام|ملی\s*گرام|ملگرام|ملیگرام|ملگرامز|ایم\s*جی|ایمجی)\b", r"\1mg"),
    ...
    # طایم / طاہم / طائم / ٹائم are ALL Whisper variants of "time"
    (r"(?:تیڈیل|ٹی\s*ڈی\s*ایس|ٹیڈیل|تین\s+دفعہ|تین\s*(?:ٹائم|طائم|طایم|طاہم|ٹائمز|طائمز)|۳\s*(?:ٹائم|طائم|طایم|طاہم)|3\s*(?:طیم|طائم|طایم|طاہم)|تین\s*مرتبہ|تین\s+بار)", "TDS"),
    (r"(?:بی\s*آئی\s*ڈی|بی\s*ڈی|صبح\s+شام|صبح\s*و\s*شام|دو\s*(?:ٹائم|طائم|طایم|طاہم|ٹائمز|طائمز)|۲\s*(?:ٹائم|طائم|طایم|طاہم)|2\s*(?:طیم|طائم|طایم|طاہم)|دو\s*مرتبہ|دو\s+بار(?!ہ|ا))", "BID"),
    (r"(?:او\s*ڈی|ایک\s+دفعہ|ایک\s*(?:ٹائم|طائم|طایم|طاہم|ٹائمز)|1\s*(?:طیم|طائم|طایم|طاہم)|ایک\s*مرتبہ|ایک\s+بار|روزانہ)", "OD"),
    ...
    (r"(?:صورت|صور|شدید|سوئر|سویئر)\b", "severe"),
